...
<body>
	...
	<div class="detailTit"><h3 align="center">소고기 상품 리스트</h3></div>
	<%-- ======================상세 페이지 이동을 위한 FORM ======================== --%>
	...
	<div id="beefList" class="col-md-5">
		...
		<c:set var="a" value="0"/>
			...
			<!-- 데이터 출력 -->
			<c:choose>
				<c:when test="${not empty beefList}">
				<c:forEach var="beef" items="${beefList}"
					varStatus="status">
				<input type="hidden" value="${beef.p_num}"/>
					<div class="col-md-4" align="center">
						<div class="row goDetail"><img src="/uploadStorage/product/${beef.p_file}" width="100px" height="100px"></div>
						<div class="row goDetail">${beef.p_name}</div>
						<div class="row goDetail">${beef.p_price}</div>
					</div>
